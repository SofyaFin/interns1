...
2) Да кто эти твои ORM методы? Это методы, которые ставят типы данных из Python в соответствие с типами данных SQL. Без них нет никакого ORM, а без ORM нам грустно :(
3) Базовый класс таблицы. Нужен в основном для двух вещей
   ...
   2) Metadata. В основном нужна для того, чтобы не записывать в env.py файл по отдельности каждую таблицу. Достаточно импортировать этот базовый класс и alembic сам разберется и найдет все изменения. Подробнее про это можно почитать в статье выше

Теперь создадим саму таблицу!
```python
...
Разберем весь этот ужас на примере строчки, начинающейся с id
 ...
 4) Integer - тип данных, хранимых в таблице. ОПЯТЬ ЖЕ, это тип данных Postgresql, помните это и сохраните себе кучу нервов
 5) primary_key=True - флаг, который говорит о том, что данная колонка - первичный ключ. Что это значит? Данные значения уникальны, поэтому по id можно ОДНОЗНАЧНО определить объект. Также, по умолчанию сортировка значений будет происходить именно по id
...
__all__ = ["Student"]
```
Первая строчка - импортирование класса таблицы Student из файла database. Вторая - список всех импортированных классов. Когда вы создаете новый класс таблицы его нужно импортировать как в примере и вписать (через запятую) в список.
...
3) -m "здесь могла быть ваша реклама" - Добавляет в название скрипта миграции текст в кавычках. Необязательный флаг, чисто для удобства.
...
1) Если у вас генерируется пустой скрипт миграции (в функциях upgrade и downgrade pass):
